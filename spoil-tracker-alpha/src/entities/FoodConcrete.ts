...
export class FoodConcreteResolver {
  ...
  async getAllFoodConcreteInPantry(
    ...
    const snapshot = await db
      .collection(COLLECTIONS.FOOD_CONCRETE)
  ...
  async createFoodConcrete(
    ...
    await pantryRef.update({
      food_concrete_items: pantryData.food_concrete_items,
    });
  ...
  async updateQuantity(
    ...
    const foodConcreteDoc = await db
      .collection(COLLECTIONS.FOOD_CONCRETE)
      .doc(food_concrete_id)
      .get();
  ...
  async deleteFoodConcrete(
    ...
    const foodConcreteRef = db
      .collection(COLLECTIONS.FOOD_CONCRETE)
      .doc(food_concrete_id);
    const foodConcreteDoc = await foodConcreteRef.get();

    if (!foodConcreteDoc.exists) {
      throw new Error(
        `FoodConcrete item with ID ${food_concrete_id} does not exist.`
      );
    }

    const foodConcreteData = foodConcreteDoc.data() as FoodConcrete;
    const pantryRef = db
      .collection(COLLECTIONS.PANTRY)
      .doc(foodConcreteData.pantry_id);
    const pantryDoc = await pantryRef.get();

    if (pantryDoc.exists) {
      const pantryData = pantryDoc.data() as Pantry;
      const updatedItems = pantryData.food_concrete_items.filter(
        (id) => id !== food_concrete_id
      );
      await pantryRef.update({ food_concrete_items: updatedItems });
    }
