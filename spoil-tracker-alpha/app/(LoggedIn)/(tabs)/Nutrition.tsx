...
export default function NutritionScreen() {
  ...
  const fetchNutritionData = useCallback(async () => {
    ...
    const goal =
      data.dailyGoals.find((g: { date: string }) => g.date === todayStr) || {};
    const log =
      data.dailyLogs.find((l: { date: string }) => l.date === todayStr) || {};

    setNutrients([
      {
        name: 'Calories',
        unit: 'kcal',
        total: Math.round((goal.caloriesGoal || 0) * 10) / 10,
      ...
      },
      {
        name: 'Protein',
        unit: 'g',
        total: Math.round((goal.proteinGoal || 0) * 10) / 10,
        consumed: log.consumedProtein || 0,
        color: '#FFBB33',
      },
      {
        name: 'Carbs',
        unit: 'g',
        total: Math.round((goal.carbsGoal || 0) * 10) / 10,
        consumed: log.consumedCarbs || 0,
        color: '#FF6666',
      },
      {
        name: 'Fats',
        unit: 'g',
        total: Math.round((goal.fatsGoal || 0) * 10) / 10,
        consumed: log.consumedFats || 0,
        color: '#66CC99',
      },
    ]);
  ...
  const handleLogFood = async (food: any) => {
    ...
    log.consumedCalories +=
      Math.round(
        (macros.protein * 4 + macros.carbohydrate * 4 + macros.total_fat * 9) *
          10
      ) / 10;
  ...
  const handleRemoveFood = async (index: any) => {
    ...
    for (const food of log.foodItems) {
      ...
      log.consumedCalories +=
        Math.round(
          (m.protein * 4 + m.carbohydrate * 4 + m.total_fat * 9) * size * 10
        ) / 10;
    ...
    >
      ...
      <View style={[styles.cardWrapper, { width: '80%' }]}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            ...
            >
              <Image
                source={{ uri: item.food_picture_url }}
        ...
        <View style={[styles.cardWrapper, { width: '80%' }]}>
          ...
          {loggedFoods.map((food, index) => (
            <View key={index} style={styles.foodCard}>
              <Image
                source={{ uri: food.food_picture_url }}
                style={styles.foodImage}
              />
              <View style={styles.foodDetails}>
                ...
                <View style={styles.macrosRow}>
                  <Text style={styles.macroText}>
                    Calories:{' '}
                    {Math.round(
                      (food.macronutrients.protein * 4 +
                        food.macronutrients.carbohydrate * 4 +
                        food.macronutrients.total_fat * 9) *
                        10
                    ) / 10}{' '}
                    kcal
                  </Text>
                ...
                <View style={styles.macroBreakdown}>
                  <Text style={styles.macroValue}>
                    ...
                    <Text style={styles.bold}>
                      {food.macronutrients.protein.toFixed(1)}g ||{' '}
                    </Text>
                  </Text>
                  <Text style={styles.macroValue}>
                    Carbs:{' '}
                    <Text style={styles.bold}>
                      {food.macronutrients.carbohydrate.toFixed(1)}g ||{' '}
                    </Text>
                  </Text>
                  <Text style={styles.macroValue}>
                    Fat:{' '}
                    <Text style={styles.bold}>
                      {food.macronutrients.total_fat.toFixed(1)}g{' '}
                    </Text>
                  </Text>
                </View>
      ...
      <View style={styles.nutrientGrid}>
        {nutrients.map((nutrient, index) => {
          ...
          const percentage =
            nutrient.total > 0 ? (nutrient.consumed / nutrient.total) * 100 : 0;
          return (
            <View key={index} style={styles.nutrientBox}>
              <Text style={styles.caloriesGoalText}>
                {nutrient.name} Goal: {nutrient.total} {nutrient.unit}
              </Text>
              ...
              <View style={styles.caloriesInfo}>
                <Text style={[styles.caloriesText, styles.rightAlign]}>
                  Consumed: {nutrient.consumed} {nutrient.unit}
                </Text>
                <Text style={[styles.caloriesText, styles.rightAlign]}>
                  Remaining: {remaining} {nutrient.unit}
                </Text>
              </View>
            </View>
          );
      ...
      <TouchableOpacity
        ...
        onPress={() => router.push('/ManageNutrition')}
      >
